...
ll mx[1005][1005], mn[1005][1005], a[1005][1005];
...
void solve()
{
    ...
    mx[1][1]=mn[1][1]=a[1][1];
    for (ll i=2; i<=m; i++) mx[1][i]=mx[1][i-1]+a[1][i], mn[1][i]=mn[1][i-1]+a[1][i];
    for (ll i=2; i<=n; i++) mx[i][1]=mx[i-1][1]+a[i][1], mn[i][1]=mn[i-1][1]+a[i][1];
    ...
    if (mx[n][m]%2==1 || mn[n][m]>0 || mx[n][m]<0) cout<<"NO\n";
    else cout<<"YES\n"; 
}
