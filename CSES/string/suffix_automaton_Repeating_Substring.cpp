...
struct SuffixAutomaton
{
    ...
    {
        vis[u]=true;
        if (st[u].len>mxlen && st[u].cnt>1 && u!=0) 
        {
            mxlen=st[u].len;
            start=st[u].firstpos-st[u].len+1;
        }
        for (auto [c, v]: st[u].next) if (vis[v]==false) dfs(v);
    }
};
...
void solve()
{
    ...
    sa.update_cnt();
    sa.dfs(0);
    if (sa.mxlen==-1) cout<<-1;
    else cout<<s.substr(sa.start, sa.mxlen);
}
