...
struct Node
{
    ...
    string ke;
    Node() {}
    Node(ll _left, ll _right, ll _sum, string _s)
    {
        left=_left;
        right=_right;
        sum=_sum;
...
ll n, m, q, cur, nNode=0, nVer=0, ver[100005], parent[100005];
...
ll update(ll l, ll r, ll x, ll y, char val, ll old, bool daobit)
{
    if (l==r)
    {
        string s=tree[old].ke;
        ll cnt=tree[old].sum;
        if (daobit==true)
        {
            cnt=0;
