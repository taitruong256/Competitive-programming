...
void solve()
{
    ...
    segment_tree dp[n+5];
    vector<ll> pref(n+5);
    for (ll i=1; i<=n; i++) dp[i].init(m+5);
    for (ll i=1; i<=n; i++)
    {
        for (ll j=1; j<=m; j++)
        {
            ll x; cin>>x;
            ll val=dp[i].query(1, 1, m, max(1LL, j-d-1), max(1LL, j-1))+x+1;
            dp[i].update(1, 1, m, j, val);
        }
        pref[i]=pref[i-1]+dp[i].query(1, 1, m, m, m);
    }
    ll ans=1e18;
    for (ll i=k; i<=n; i++) ans=min(ans, pref[i]-pref[i-k]);
    cout<<ans<<endl;
}
