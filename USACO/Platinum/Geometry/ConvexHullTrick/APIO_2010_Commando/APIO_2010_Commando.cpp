...
struct CHT 
{
    ...
    void insert(line l)   //he so goc a tang dan nen them vao ben phai cua convex hull 
    {              
...
ll n, A, B, C, a[1000005], pref[1000005], dp[1000005], ans;
...
void solve()
{
    ...
    for (ll i=1; i<=n; i++)
    {
        dp[i]=cht.query_O_logn(pref[i])+A*pref[i]*pref[i]+B*pref[i]+C;
        ans=max(ans, dp[i]);
        cht.insert({-2*A*pref[i], dp[i]+A*pref[i]*pref[i]-B*pref[i]});     
    }      
    cout<<ans;
}
