...
struct LiChao_tree       //offline + get min
{
    ...
    LiChao_tree(vector<ll> _compress_x)
    {
        ...
        tree.resize(4*n+5, {0, LLONG_MIN});       //LLONG_MIN
    }
    ...
    ll query(ll id, ll l, ll r, ll x)
    {
        ...
        ll m=(l+r)/2;
        if (x<=m) return max(tree[id].get(compress[x]), query(id*2, l, m, x));   //max()
        return max(tree[id].get(compress[x]), query(id*2+1, m+1, r, x));   //max()
    }
...
void solve()
{
    ...
    sort(rec+1, rec+n+1);
    vector<ll> comp;
    for (ll i=1; i<=n; i++) comp.push_back(rec[i].y);
    LiChao_tree seg(comp);
    seg.insert(0, 0);
    for (ll i=1; i<=n; i++)
    {
        dp[i]=seg.query(rec[i].y)+rec[i].x*rec[i].y-rec[i].a;
        seg.insert(-rec[i].x, dp[i]);   
        ans=max(ans, dp[i]);
    }
    cout<<ans;
}
