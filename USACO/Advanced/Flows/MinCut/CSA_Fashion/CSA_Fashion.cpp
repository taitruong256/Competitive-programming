...
void solve()
{
    ...
    {
        ll t, c; cin>>t>>c;
        dinic.add_edge(m+i, n+m+1, c);
    }  
    for (ll i=1; i<=m; i++)
    {
        ll a, b, c, w; cin>>a>>b>>c>>w;
        ans+=w;
        dinic.add_edge(0, i, w);
        dinic.add_edge(i, m+a, 1e18);
        dinic.add_edge(i, m+b, 1e18);
        dinic.add_edge(i, m+c, 1e18);
    }
    cout<<ans-dinic.maxFlow()<<" ";
    vector<ll> res;
    for (ll i=1; i<=n; i++) if (dinic.level[i+m]!=-1) res.push_back(i);
    cout<<res.size()<<endl;
