...
void solve()
{
    ...
    {
        cin>>a[i];
        if (a[i]>0) dinic.add_edge(0, i, a[i]), ans+=a[i];
        else 
        {
            dinic.add_edge(i, n+1, -a[i]);
            for (ll j=i+i; j<=n; j+=i) dinic.add_edge(j, i, 1e18);
        }
    }
    for (auto [u, v, f, c]: dinic.edge) cout<<u<<" "<<v<<" "<<c<<endl;
