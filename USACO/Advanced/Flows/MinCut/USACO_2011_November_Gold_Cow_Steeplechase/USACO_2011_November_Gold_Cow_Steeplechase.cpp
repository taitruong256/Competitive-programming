...
bool intersec(Ngang a, Doc b)
{
    if (a.x1<=b.x && b.x<=a.x2 && b.y1<=a.y && a.y<=b.y2) return true;
    return false;
}
...
void solve()
{
    ...
    for (ll i=1; i<=n; i++)
    {
        ...
        if (x1>x2) swap(x1, x2);
        if (y1>y2) swap(y1, y2);
        if (x1==x2) doc.push_back({x1, y1, y2});
        else ngang.push_back({x1, x2, y1});
    }
    ll n1=ngang.size(), n2=doc.size();
    for (ll i=0; i<n1; i++)
        for (ll j=0; j<n2; j++)
            if (intersec(ngang[i], doc[j])==true)
                dinic.add_edge(i+1, n1+j+1, 1);
    for (ll i=1; i<=n1; i++) dinic.add_edge(0, i, 1);
    for (ll i=1; i<=n2; i++) dinic.add_edge(n1+i, n+1, 1);
    cout<<n-dinic.maxFlow();
}
