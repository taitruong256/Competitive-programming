...
void solve()
{
    ...
    for (ll i=1; i<=n; i++) dinic.add_edge(0, i, 1);
    for (ll i=1; i<=m; i++) dinic.add_edge(n+i, n+m+1, 1);
    cout<<dinic.maxFlow()<<endl;
    // for (auto [u, v, flow, cap]: dinic.edge) if (flow==1 && 1<=u && u<=n) cout<<u-1<<" "<<v-n-1<<endl;
}
