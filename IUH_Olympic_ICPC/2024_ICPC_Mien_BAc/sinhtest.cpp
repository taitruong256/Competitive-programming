...
string code = "testG";    // nhap ma de bai tren iuhcoder vao day
void genInput(const char *cFileIn){        //ghi testcase vao file input
    freopen(cFileIn, "w", stdout);
    int n=randint(5, 20);
    printf("%d\n", n);
    for (int i=0; i<n; i++) printf("%d", randint(1, 100)%2);
}
void solve(){      //loi giai cua van de
    ...
    {
        string t;
        for (int j=0; j<n; j++) if (i&(1<<j)) t.push_back('1');
        else t.push_back('0');
        t="1"+t+"1";
        vector<int> v;
        int cnt=0;
        ...
        int pre=v[0];
        bool check=1;
        for (auto i: v) if (i!=pre) check=0;
        if (check==1) ans=max(ans, pre*(int)v.size()+(int)v.size()-1);
    }
    printf("%d", ans);
}
int main(int argc, char *argv[]) {
    int Start = 1; 
    int End =20;  //tao cac test case bat dau tu Start den End
    fflush(stdin);
    srand(time(0));
    bool isGenInp = true;
