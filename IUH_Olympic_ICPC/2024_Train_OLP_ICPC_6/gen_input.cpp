...
void gen_input(int seed) {
    ...
    int rows=randint(20, 500), cols=randint(20, 500), xx=randint(1, rows), yy=randint(1, cols);
    cout<<rows<<" "<<cols<<" "<<xx<<" "<<yy<<endl;
    int n=randint(30, 100);
    cout<<n<<endl;
    for (int i=1; i<=n; i++)
    {
        while (true)
        {   
            int x=randint(1, rows), y=randint(1, cols);
            if (x!=xx && y!=yy) 
            {
                cout<<x<<" "<<y<<endl;
                break;
    ...
    int q=randint(30, 100);
    cout<<q<<endl;
    while (q--)
    {
        int type=randint(1, 100)%4, d=randint(1, 100);
        if (type==0) cout<<"L "<<d<<endl;
        else if (type==1) cout<<"R "<<d<<endl;
        else if (type==2) cout<<"U "<<d<<endl;
        else if (type==3) cout<<"D "<<d<<endl;
    }
}
